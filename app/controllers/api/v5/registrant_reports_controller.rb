...
class Api::V5::RegistrantReportsController < Api::V5::BaseController
  def create
    query = {
    ...
    jsonp V5::RegistrationService.create_report(query)
  ...
  def show
    ...
    jsonp V5::RegistrationService.get_report(query)
  rescue ArgumentError => e
    jsonp({ :message => e.message }, :status => 400)        
  end
  ...
  def get_report(id, api_key)
    partner = V5::PartnerService.find_partner(id, api_key)
    report = Report.find_by_id(params[:id])
    if report.nil? || report.partner != partner
      raise(ArgumentError.new(V5::RegistrationService::REPORT_PERMISSIONS_ERROR)) if report.partner != partner
    end
    return report
  end
