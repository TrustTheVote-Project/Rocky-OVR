class TranslationsController < ApplicationController
  ...
  def preview_pdf
    ...
    url = @registrant.registration_instructions_url
    if !url.blank?
      @registrant_instructions_link = "<br><a href='#{url}'>#{url}</a><br>"
    else
...
private
  ...
  def submit_data(file, group_name, locale)
    ...
    #Send an email
    ConfigMailer.translation_file(file, "#{group_name}-#{locale}.yml").deliver_now
