module AbrStateMethods::LA
  ...
  PDF_FIELDS = {
    ...
    'abr_reason_selections':{options:["abr_reason2", "abr_reason3", "abr_reason5", "abr_reason6", "abr_reason7","abr_reason8","abr_reason9","abr_reason10"]},
    ...
    'abr_party_selections':{options:["abr_party1", "abr_party2", "abr_party3"]},
    'abr_assistant_name':{},
    'abr_relationship1':{},
...
#'abr_election_type1_input', 'abr_election_type2_input']
  EXTRA_FIELDS = ['abr_absence_begin_date_input','abr_absence_begin_date_input_mm','abr_absence_begin_date_input_dd','abr_absence_begin_date_input_yyyy','abr_absence_end_date_input', 'abr_absence_end_date_input_mm','abr_absence_end_date_input_dd','abr_absence_end_date_input_yyyy',
  'abr_mailing_address_line_1_input',  'abr_mailing_city_input', 'abr_mailing_state_abbrev_input', 'abr_mailing_zip_input', 'abr_mailing_unit_input', 'abr_mother_maiden_input',
'abr_election_type1_input', 'abr_election_type2_input', 'abr_party_selections']
  ...
  def form_field_items
    ...
    {'abr_last_4_ssn':{}},
    {'abr_reason_selections':{type: :radio, required:true, options:["abr_reason2", "abr_reason3", "abr_reason5", "abr_reason6", "abr_reason7","abr_reason8","abr_reason9","abr_reason10"]}},
    ...
    {'abr_election_type1_input':{type: :radio, required: false, options:["abr_primary_none", "abr_primary1", "abr_primary2", "abr_primary3"]}},
    {'abr_election_type2_input':{type: :radio, required: false, options:["abr_general_none", "abr_general1", "abr_general2", "abr_general3"]}},
    {'abr_party_selections':{type: :radio, required: false, options:["abr_party1", "abr_party2", "abr_party3"]}},
    {'abr_assistant_name':{}},
    {'abr_relationship1':{}},
 ...
 def custom_form_field_validations
  ...
  # Must select at least one election (primary or general)
  primary_selected = !["", "abr_primary_none"].include?(abr_election_type1_input.to_s)
  general_selected = !["", "abr_general_none"].include?(abr_election_type2_input.to_s)
  if !primary_selected && !general_selected
 ...
 def test_date(datestring)
  ...
  # Returns the primary election date based on the selected option
  def abr_election_type1_string
    case self.abr_election_type1_input.to_s
    when "abr_primary1" then "03/14/26"
    when "abr_primary2" then "05/16/26"
    when "abr_primary3" then "11/03/26"
    end
  end

  # Returns the general election date based on the selected option
  def abr_election_type2_string
    case self.abr_election_type2_input.to_s
    when "abr_general1" then "04/18/26"
    when "abr_general2" then "06/27/26"
    when "abr_general3" then "12/12/26"
    end
  end

  # Only populate Ballot Delivery Address if user checked the mailing address box
  def insure_abr_mailing_address_line_1
    return self.abr_mailing_address_line_1_input if self.abr_check_mailing_address.to_s == "1"
  end

  def insure_abr_mailing_unit
    return self.abr_mailing_unit_input if self.abr_check_mailing_address.to_s == "1"
  end

  def insure_abr_mailing_city
    return self.abr_mailing_city_input if self.abr_check_mailing_address.to_s == "1"
  end

  def insure_abr_mailing_state_abbrev
    return self.abr_mailing_state_abbrev_input if self.abr_check_mailing_address.to_s == "1"
  end

  def insure_abr_mailing_zip
    return self.abr_mailing_zip_input if self.abr_check_mailing_address.to_s == "1"
  end
end
