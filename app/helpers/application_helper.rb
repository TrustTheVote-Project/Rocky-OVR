module ApplicationHelper
  ...
  def radio_div(form, field, radio_options, options={})
    ...
    radio_buttons = radio_options.collect do |text, value|
      disabled = options[:disabled] && options[:disabled].include?(value)
      radio = form.radio_button(field, value, {disabled: disabled}).html_safe
      form.label("#{field}_#{Abr.make_method_name(value, prefix_numbers: false)}", "#{radio} #{text}".html_safe, class: disabled ? 'disabled' : '').html_safe
    end.join("\n").html_safe
