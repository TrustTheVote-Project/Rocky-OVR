...
describe Step2Controller do
  ...
  describe "#update" do
    ...
    it "should update registrant and complete step 2 when not using short form" do
      put :update, params: {:registrant_id => @registrant.to_param, :registrant => FactoryGirl.attributes_for(:step_2_registrant, home_zip_code: "03900").reject {|k,v| k == :status }.merge(short_form: 0)}
    ...
    it "should update registrant and complete after step 2" do
      put :update, params: {:registrant_id => @registrant.to_param, :registrant => FactoryGirl.attributes_for(:step_5_registrant, home_zip_code: "03900").reject {|k,v| k == :status }}
      assert !assigns[:registrant].nil?
    ...
    it "should reject invalid input and show form again" do
      put :update, params: {:registrant_id => @registrant.to_param, :registrant => FactoryGirl.attributes_for(:step_2_registrant, :first_name => nil, home_zip_code: "03900").reject {|k,v| k == :status }}
    ...
    it "should reject invalid input and show form again when not using short form" do
      put :update, params: {:registrant_id => @registrant.to_param, :registrant => FactoryGirl.attributes_for(:step_2_registrant, :first_name => nil, home_zip_code: "03900").reject {|k,v| k == :status }.merge(short_form: 0)}
      assert assigns[:registrant].step_2?
      assert assigns[:registrant].reload.step_1?
      assert_template "show"
    end
    ...
    context "if using a short form" do 
      ...
      it "should go to step 4 if on ovr_flow" do
        ...
        put :update, params: {
          :registrant_id => @registrant.to_param, :registrant => FactoryGirl.attributes_for(:maximal_registrant).reject {|k,v| k == :status }
        }
        assert !assigns[:registrant].nil?
