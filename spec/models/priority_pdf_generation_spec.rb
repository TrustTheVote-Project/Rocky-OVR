...
describe PriorityPdfGeneration do
  ...
  describe '.retrieve_from_queue' do
    ...
    before(:each) do
      ...
      PdfDeliveryGeneration.stub(:receive_and_generate)
    end
    ...
    context 'No rows avaialable' do
      before(:each) do
        receive_resp.stub(:messages).and_return([])
        PriorityPdfGeneration.stub(:sleep)
      end
      it "calls PdfDeliveryGeneration.retrieve_from_queue" do
        PdfDeliveryGeneration.should_receive(:receive_and_generate)
        PriorityPdfGeneration.retrieve_from_queue
      end
