...
describe V2::StateRequirements do

  context 'error' do
    it 'if the state is not found' do
      lambda {
        V2::StateRequirements.find(:home_state_id => 'ZZ')
      }.should raise_error V2::StateRequirements::INVALID_STATE_ID
    end

    it 'if the zip code is invalid' do
      lambda {
        V2::StateRequirements.find(:home_zip_code => '00000')
      }.should raise_error V2::StateRequirements::INVALID_ZIP
    end

    it 'if the zip code has invalid format' do
      lambda {
        V2::StateRequirements.find(:home_zip_code => '06390abc')
      }.should raise_error V2::StateRequirements::INVALID_ZIP
    end

    it 'if the zip does not match the state' do
      lambda {
        V2::StateRequirements.find(:home_state_id => 'AK', :home_zip_code => '06390')
      }.should raise_error V2::StateRequirements::NO_ZIP_MATCH
    end

    it 'if neither state ID nor ZIP code are given' do
      lambda { V2::StateRequirements.find({}) }.should raise_error V2::StateRequirements::MISSING_ID_OR_ZIP
    end

    it 'if birth date has invalid format' do
      lambda {
        V2::StateRequirements.find(:home_state_id => 'CA', :lang => 'en', :date_of_birth => 'invalid_format')
      }.should raise_error V2::StateRequirements::BAD_DOB_FORMAT
    end

    it 'if age is invalid' do
      lambda {
        V2::StateRequirements.find(:home_state_id => 'CA', :lang => 'en', :date_of_birth => 5.days.ago.strftime('%Y-%m-%d'))
    ...
    it 'if the language is unsupported' do
      lambda {
        V2::StateRequirements.find(:home_state_id => 'AK', :lang => 'ru')
  ...
  it 'should return data' do
    ...
    V2::StateRequirements.find(:home_state_id => 'CA', :lang => 'en').should == {
      :requires_party     => true,
      :id_number_msg      => nil,
