...
describe V4::PartnerService do
  ...
  describe "#find" do
    context 'error' do
      it 'raises an error if the partner is not found' do
        lambda {
          V4::PartnerService.find(:partner_id => 0)
      ...
      it 'raises an error if the api key is invalid' do
        ...
        lambda {
          V4::PartnerService.find(:partner_id => partner.id, :partner_api_key=>nil)
        }.should raise_error V4::PartnerService::INVALID_PARTNER_OR_API_KEY
      end
    end
    it 'should return all data and all columns' do
      ...
      response = V4::PartnerService.find(:partner_id => partner.id, :partner_api_key => partner.api_key)
    ...
    it 'should return only public data' do
      ...
      V4::PartnerService.find({ :partner_id => partner.id, :partner_api_key => partner.api_key }, true).should == expected_response
    end
  end

  describe '#data_to_attrs' do
    specify { V4::PartnerService.send(:data_to_attrs, {:org_name=>"Name"}).should == {:organization=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:org_URL=>"Name"}).should == {:url=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:org_privacy_url=>"Name"}).should == {:privacy_url=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:logo_image_URL=>"Name"}).should == {:logo_url=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_email=>"Name"}).should == {:email=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_name=>"Name"}).should == {:name=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_address=>"Name"}).should == {:address=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_city=>"Name"}).should == {:city=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_phone=>"Name"}).should == {:phone=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_state=>"nJ"}).should == {:state_id=>31} }
    specify { V4::PartnerService.send(:data_to_attrs, {:contact_ZIP=>"Name"}).should == {:zip_code=>"Name"} }
    specify { V4::PartnerService.send(:data_to_attrs, {:partner_ask_volunteer=>true}).should == {:partner_ask_for_volunteers=>true} }
    #banner_image_URL
  end
  ...
  describe "#create" do
    it 'should raise an error if the field is unknown' do
      begin
        V4::PartnerService.create_record(:unknown_field => 'fieldval')
    ...
    it 'should raise validation errors when the record is invalid' do
      begin
        V4::PartnerService.create_record({})
    ...
    it 'does not allow parameters except the expected ones to be set' do
      begin
        V4::PartnerService.create_record({:survey_question_1=>true})
        fail 'UnknownAttributeError expected'
      rescue ActiveRecord::UnknownAttributeError => e
    ...
    context 'complete record' do
      ...
      it 'should save the record' do
        p = V4::PartnerService.create_record(params)
        ...
        V4::PartnerService.create_record(params2).should
      end
    end
